...
cd /d "%~dp0"
set "PID_FILE=%~dp0opencoursedeck-server.pid"
set "DEBUG_LOG=%~dp0debug.log"

for /f %%t in ('powershell -NoProfile -Command "Get-Date -Format o"') do set "STAMP=%%t"
>> "%DEBUG_LOG%" echo {"location":"Run-OpenCourseDeck.cmd","message":"starting","timestamp":"%STAMP%"}

if not exist "node_modules\esbuild\package.json" (
  echo [OpenCourseDeck] Dependencies are missing. Run npm install first.
  exit /b 1
)
