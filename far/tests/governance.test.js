...
const farRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const repoRoot = path.dirname(farRoot);
...
describe('repository governance', () => {
  ...
  it('documents immutable release recovery and forward rollback', () => {
    const runbook = fs.readFileSync(path.join(farRoot, 'docs/release-and-rollback.md'), 'utf8');
